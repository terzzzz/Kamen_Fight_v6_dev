...
(function (window) {
  ...
  /** Cycles CPU difficulty setting through 4 levels (easy -> balanced -> master -> soul). */
  ...
  /**
   * Opens simulation modal, runs batch simulations, and executes RL training when Ichigo is set to SOUL.
   */
  window.handleSimulateMatches = function(e) {
    ...
    // SOUL difficulty triggers Ichigo RL self-learning
    const p1IsIchigoSoul = p1Rider.id === 'ichigo' && p1Diff === 'soul';
    const p2IsIchigoSoul = p2Rider.id === 'ichigo' && p2Diff === 'soul';
    const isIchigoTraining = (p1IsIchigoSoul || p2IsIchigoSoul) && window.AgentIchigo && typeof window.AgentIchigo.evolveForOpponent === 'function';

    resultsBody.innerHTML = `
      <p class="sim-loading" style="color: #00ffcc; text-align: center; font-family: monospace; padding: 20px; font-size: 1.1rem;">
        ${isIchigoTraining ? '🧠 TRAINING ICHIGO VIA EVOLUTIONARY RL (SOUL LEVEL)...' : `SIMULATING ${matchCount} MATCHES... PLEASE WAIT...`}<br>
    ...
    setTimeout(async () => {
      try {
        if (isIchigoTraining) {
          const opponentId = p1IsIchigoSoul ? p2Rider.id : p1Rider.id;
          console.log(`[AgentIchigo] Starting SOUL RL evolution vs ${opponentId}...`);
          await window.AgentIchigo.evolveForOpponent(opponentId, {
            generations: 10,
            popSize: 8,
            matchesPerEval: Math.max(10, Math.floor(matchCount / 2))
          });
        }
        ...
        resultsBody.innerHTML = `
          <div class="sim-summary-header" style="text-align: center; margin-bottom: 15px; font-family: monospace;">
            ...
            ${isIchigoTraining ? '<p style="color:#d870ff; font-size:0.85rem; margin-top:4px;">✨ SOUL weights trained & saved to LocalStorage! Click EXPORT to download JSON.</p>' : ''}
          </div>
