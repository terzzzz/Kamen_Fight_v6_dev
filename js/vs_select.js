...
(function (window) {
  ...
  window.handleSimulateMatches = function(e) {
    ...
    if (!modal) {
      ...
      modal.innerHTML = `
        <div class="sim-modal-content">
          ...
          <div style="display: flex; justify-content: center; gap: 10px; margin-top: 15px;">
            <button id="btn-export-ichigo" class="nav-btn" style="background:#0088cc; color:#fff; border:1px solid #00aaff; cursor:pointer;">
      ...
      // ASYNC EXPORT FIX: Await policy store before triggering file download
      const exportBtn = modal.querySelector('#btn-export-ichigo');
      if (exportBtn) {
        exportBtn.addEventListener('click', async () => {
          if (!window.AgentIchigo) {
            alert('AgentIchigo engine is not loaded!');
            return;
          }
          try {
            await window.AgentIchigo.loadActivePolicyStore();
            window.AgentIchigo.exportWeights();
          } catch (err) {
            console.error('[AgentIchigo] Export error:', err);
